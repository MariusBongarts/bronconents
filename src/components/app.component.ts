...
export class AppRoot extends LitElement {
  ...
  render() {
    return html`
    <bronco-template .hideNav=${false}>
      <div slot="header">
        <bronco-top-navbar .searchArray=${this.navItems} .hideOnNotTop=${true}
        @selected=${(e: CustomEvent) => this.scrollToSection(e.detail)}>

          <img src="${logoSvg}" slot="left" tabindex="0">

          <bronco-searchbar
          .searchArray=${this.navItems}
          @selected=${(e: CustomEvent) => this.scrollToSection(e.detail)} slot="center"></bronco-searchbar>
